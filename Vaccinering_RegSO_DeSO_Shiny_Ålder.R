    ...
    mainPanel(   
      tabsetPanel(type = "tabs",
                  ...
                  tabPanel("Aktuellt läge (per RegSO pch ålder)", 
                           plotOutput("kartaAktuelltLäge_Ålder", width = "100%", height = "800px"),
                           htmlOutput("tabellAktuelltLäge_Ålder")
                  ),
                  tabPanel("Förändring sedan föregående mätning (per RegSO och ålder)", 
                           plotOutput("kartaFörändring_Ålder", width = "100%", height = "800px"),
                           htmlOutput("tabellFörändring_Ålder")
                  )
      )
    )
  )
...
server <- function(input, output) {
  ...
  output$kartaAktuelltLäge_Ålder <- renderPlot({
  ...
  output$tabellAktuelltLäge_Ålder <- renderUI({
  ...
  output$kartaFörändring_Ålder <- renderPlot({
    sf_result_regso_ålder_filtered()  %>%
      ggplot() + 
      geom_sf(aes(fill = Skillnad.fg.månad))+  
      {if(input$kommun != "Alla") geom_label_repel(aes(label = RegSO,  x = RegSO_x, y = RegSO_y), force=50, size=4, max.overlaps = 1000, color = "black")   }+
  ...
  output$tabellFörändring_Ålder <- renderUI({
    sf_result_regso_ålder_filtered_kommun()  %>%
      as.data.table() %>%
      addHtmlTableStyle(align = "r") %>%
